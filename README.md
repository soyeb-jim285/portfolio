...
- Input size, history size, output tokens, request timeout, concurrency, per-address minute limits and a daily request count are bounded. The daily count lives in the database, so it is shared across processes and survives restarts; the per-minute burst limit and concurrency cap are still per process. None of these is a dollar budget, so set a provider-side spending limit before public deployment.
- The API uses the socket peer IP, not caller-controlled forwarded headers. Behind a reverse proxy, visitors share the proxy's peer limit; configure per-visitor rate limiting at that trusted proxy before launch. Origin checks are not authentication.
...
- Remaining Phase 1 follow-ups: a build-time knowledge export so the API does not import frontend TypeScript, a shared limiter if more than one API replica runs, and production deployment verification. Resend and R2 settings stay commented out until their phases.
